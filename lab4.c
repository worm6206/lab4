...
{ 
  ...
  glUniform4f(Kd, 0.7, 0.5, 0.7, 1); 
  draw_cylinder(mvp, mv, &view[0][0], c0, c2, m1, m2, m3, m4); 
...
// light and floor
  ...
  floorM = glm::scale(floorM, glm::vec3(15.0, 15.0, 1.0));

  mvp = projection*view*floorM;
  mv = view*floorM;

  glUniform4f(Kd, 0.2, 0.7, 0, 1); 
  draw_cube(mvp, mv, &view[0][0], c0, c2, m1, m2, m3, m4);
...
// objects for lighting demo
  ...
  modelM = glm::scale(modelM, glm::vec3(4.0f, 4.0f, 1.0f));
  mvp = projection*view*model*modelM;
  mv = view*model*modelM;
  
  glUniform4f(Kd, 0.7, 0.2, 0, 1); 
  draw_cylinder(mvp, mv, &view[0][0], c0, c2, m1, m2, m3, m4); 
  ...
  modelM = glm::rotate(modelM, glm::radians(angle1), glm::vec3(0.0f, 0.0f, 1.0f));
  modelM = glm::translate(modelM, glm::vec3(2.0f, 0.0f, 0.0f));  
  
  mat_stack.push(modelM);  
  modelM = glm::scale(modelM, glm::vec3(3.0f, 3.0f, 3.0f));
  mvp = projection*view*model*modelM;
  mv = view*model*modelM;
